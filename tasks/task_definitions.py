...
def create_backend_task(feature_description: str) -> Task:
    return Task(
        description=(
            ...
            "Extract PROJECT_SLUG and PLATFORM from the first two lines of the architecture "
            "output (line 1: `PROJECT_SLUG: <slug>`, line 2: `PLATFORM: web|mobile|universal`) "
            "and use them for all file paths and configuration decisions.\n\n"
            ...
            "- CORS middleware (platform-aware):\n"
            "    web:              allow_origins=['http://localhost:3000', '<production-url>']\n"
            "    mobile/universal: allow_origins=['*'] in development (React Native apps do\n"
            "                      not run in a browser so browser CORS is not enforced;\n"
            "                      restrict to specific API gateway URL in production)\n"
...
def create_dev_gate_task(feature_description: str, sprint: int = 1) -> Task:
    return Task(
        description=(
            ...
            "Extract PLATFORM from ARCHITECTURE.md and use it for file-presence checks. "
            "Decide whether the implementation is ready to move to testing.\n\n"
            "## Required files check (by PLATFORM):\n"
            "  web:       frontend/src/app/page.tsx, frontend/next.config.mjs, "
            "frontend/tailwind.config.ts\n"
            "  mobile:    mobile/app/_layout.tsx, mobile/tamagui.config.ts, "
            "mobile/app.json, mobile/babel.config.js\n"
            "  universal: all mobile files above + 'web' in app.json platforms array\n\n"
            "## APPROVE if:\n"
            "- Code review is REVIEW SUMMARY: CLEAN, or only Minor issues remain\n"
            "- All required files for the detected PLATFORM are present\n"
            "- Full API contract is implemented\n\n"
            "## REJECT if:\n"
            "- Any Critical security or correctness issue exists\n"
            "- More than 2 Major issues exist\n"
            "- Platform-specific required files are missing\n\n"
...
def create_test_gate_task(feature_description: str, sprint: int = 1) -> Task:
    return Task(
        description=(
            ...
            "Extract PLATFORM from ARCHITECTURE.md. "
            "Decide whether the test results are acceptable to proceed to DevOps.\n\n"
            "## Test framework by PLATFORM:\n"
            "  web:              Vitest (frontend/__tests__/) + pytest (backend/tests/)\n"
            "  mobile/universal: Jest + jest-expo (mobile/__tests__/) + "
            "pytest (backend/tests/)\n\n"
            "## APPROVE if:\n"
            "- All tests pass, or only Minor/cosmetic failures remain\n"
            "- Every API endpoint has pytest coverage\n"
            "- Frontend/mobile components have test coverage (Vitest or Jest)\n"
            ...
            "- No frontend/mobile test files present at all\n\n"
            "## MANDATORY — your Final Answer MUST begin with:\n"
            "VERDICT: APPROVED  or  VERDICT: REJECTED\n\n"
